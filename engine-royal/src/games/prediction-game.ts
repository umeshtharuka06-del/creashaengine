...
export async function settleDuePredictionRounds(mode: PredictionMode) {
  ...
  try {
    due = await prisma.gameRound.findMany({
      // state MUST be an equality/`$in`, NEVER `{ not: "SETTLED" }`. `$ne` cannot
      // seek the `@@index([game, state])`, so at scale it scans EVERY round of the
      // game — including the millions already SETTLED — on every tick, which is too
      // slow to complete and silently stalls settlement (the real freeze). `$in`
      // on the only unsettled states seeks just those rounds → O(unsettled).
      where: { game: mode, state: { in: ["BETTING", "RUNNING"] }, settleAt: { lte: now } },
      // No `include:{bets:true}` — bets are aggregated server-side per round so
      // memory stays O(options). Newest due first so the just-expired round settles
      // on the next tick; a backlog still drains fully (top 50 leave the set each tick).
